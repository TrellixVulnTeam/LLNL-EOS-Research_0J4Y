...
if __name__ == '__main__':
    ...
    for element in element_list:
        ...
        purg_y = np.log(
            np.asarray([library_element.pressure_data[i] for i in reversed(var_domain)], dtype=float)) / np.log(10)
    ...
    expanded_x_list = np.asarray([x_list[0] + dx * i for i in range(max(shift_index_dict.values()) + len(x_list))],
                                 dtype=float)
    ...
    # expanded_x_list, collapsed_y = expanded_x_list[shift_index_dict['He']:-cut], collapsed_y[shift_index_dict['He']:-cut]
    # expanded_x_list, collapsed_y = expanded_x_list[shift_index_dict['He']:-cut], collapsed_y[shift_index_dict['He']:-cut]

    pyplot.subplot(143)
    pyplot.plot(expanded_x_list, collapsed_y, color='black')
    
    pyplot.subplot(144)
    pyplot.plot(np.power(10, expanded_x_list), collapsed_y, color='black')

    A = np.vstack((expanded_x_list[320:], np.ones(len(expanded_x_list) - 320))).T
    print(np.linalg.lstsq(A, np.log(collapsed_y[320:]), rcond=None)[0])
